...
def api_rolling_earnings():
    ...
    if payload is None:
        try:
            ...
            # 上游(forecast_browse/runup 等 PC 端导出)尚未生成, 属正常空态, 不算服务端错误
            return jsonify({"items": [], "n": 0, "rolling": {"items": []}, "message": f"暂无滚动业绩数据: {e}"})
        except Exception as e:
...
def run_daily_update(attempt: int = 1):
    ...
    try:
        from scripts.update_daily import UpdateAlreadyRunning, main as update_main
        update_main()
        ...
        # 另一个实例(web 容器手动刷新等)正在跑同一套数据, 属正常互斥, 不算失败
        finished_at = datetime.now().isoformat(timespec="seconds")
        log.info("daily update skipped, another instance holds the lock: %s", e)
        try:
            _write_daily_update_status(
                "skipped",
                attempt,
            ...
            log.exception("failed to persist daily update skip status")
    except Exception as e:
        finished_at = datetime.now().isoformat(timespec="seconds")
        try:
            _write_daily_update_status(
...
def _refresh_all_data() -> None:
    """Run full data refresh: daily market data, then weekly financials."""
    global _refresh_all_state
    with _refresh_all_lock:
        if _refresh_all_state["running"]:
            return
        _refresh_all_state["running"] = True
        _refresh_all_state["started_at"] = datetime.now().isoformat(timespec="seconds")
        _refresh_all_state["finished_at"] = None
        _refresh_all_state["last_error"] = None
        _refresh_all_state["current_step"] = "daily_update"

    def _finish(error: str | None = None) -> None:
        global _refresh_all_state
        with _refresh_all_lock:
            _refresh_all_state["running"] = False
            _refresh_all_state["finished_at"] = datetime.now().isoformat(timespec="seconds")
            _refresh_all_state["last_error"] = error
            _refresh_all_state["current_step"] = None

    try:
        log.info("=== refresh all data started ===")
        run_daily_update(attempt=1)
        with _refresh_all_lock:
            _refresh_all_state["current_step"] = "weekly_financials"
        run_weekly_financials_update()
        log.info("=== refresh all data succeeded ===")
        _finish(error=None)
    except Exception as e:
        log.exception("refresh all data failed: %s", e)
        _finish(error=f"{type(e).__name__}: {e}")


@app.route("/api/refresh_all", methods=["POST"])
def api_refresh_all():
    """Trigger a full backend data refresh in a background thread."""
    if _refresh_all_state["running"]:
        return jsonify({
            "ok": False,
            "running": True,
            "message": "全量刷新已在进行中",
            "started_at": _refresh_all_state.get("started_at"),
        })
    thread = threading.Thread(target=_refresh_all_data, daemon=True)
    thread.start()
    return jsonify({
        "ok": True,
        "running": True,
        "message": "已启动全量数据刷新（后台执行，可能耗时较久）",
        "started_at": _refresh_all_state.get("started_at"),
    })


@app.route("/api/refresh_all/status", methods=["GET"])
def api_refresh_all_status():
    """Get the current state of the full data refresh job."""
    return jsonify({
        "running": _refresh_all_state["running"],
        "started_at": _refresh_all_state.get("started_at"),
        "finished_at": _refresh_all_state.get("finished_at"),
        "last_error": _refresh_all_state.get("last_error"),
        "current_step": _refresh_all_state.get("current_step"),
    })
...
def boot_freshness_catchup():
    ...
    def _check():
        try:
            ...
            if dfc is None or getattr(dfc, "empty", True) or "cal_date" not in dfc.columns:
                log.warning("启动检查: trade_cal 未返回交易日历(tushare 临时异常), 跳过本次新鲜度检查")
                return
